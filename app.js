...
function genRoast(input, target){
  ...
  var roastLine = fill(pickVaried('rat_roast', RAT_ROAST), { target:target, action: shortInput(input,12) });
...
function genSelfmock(input){
  ...
  var bit = fill(pickVaried('rat_selfmock', RAT_SELFMOCK), { target:'生活' });
  var quote = pickGoldenQuote();
  return {
    role:'rat', tagClass:'vent tag-rat',
    blocks:[
...
function runGenerate(id){
  ...
  if(id === 'roast'){
    ...
    var emo = getChipValue('emotion-chip');
